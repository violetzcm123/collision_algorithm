...
namespace UnityNativeHull
{
    public unsafe struct NativeHull : IDisposable
    {
        ...
        //  NativeArray 在构造时跳过 LeakDetection 注册，用来提高速度。
        ...
        [NativeDisableUnsafePtrRestriction] public float3* Vertices; // 顶点指针

        [NativeDisableUnsafePtrRestriction] public NativeFace* Faces; // 面指针

        [NativeDisableUnsafePtrRestriction] public NativePlane* Planes; // 平面指针

        [NativeDisableUnsafePtrRestriction] public NativeHalfEdge* Edges; // 半边指针
        ...
        // 获取半边
        public unsafe NativeHalfEdge GetHalfEdge(int index) => EdgesNative[index];

        // 获取半边引用
        public unsafe ref NativeHalfEdge GetHalfEdgeRef(int index) => ref *(index + Edges);

        // 获取半边指针
        public unsafe NativeHalfEdge* GetHalfEdgePtr(int index) => Edges + index;

        // 获取面（有限的真实的面，具体的矩形，三角形之类的面）
        public unsafe NativeFace GetFace(int index) => FacesNative[index];

        // 获取面指针
        public unsafe NativeFace* GetFaces(int index) => Faces + index;
    ...
    public static class NativeHullExtensions
    {
        ...
        public static float3 CalculateFaceCentroid(this NativeHull hull, NativeFace face)
        {
            float3 center = 0;
            int edgeCount = 0;
            ref NativeHalfEdge start = ref hull.GetHalfEdgeRef(face.Edge);
            ref NativeHalfEdge current = ref start;

            do
            {
                edgeCount++;
                center += hull.GetVertex(current.Origin);
                current = ref hull.GetHalfEdgeRef(current.Next);
            } while (current.Origin != start.Origin);

            return center / edgeCount;
        }
    }
}